...
export async function POST(
    ...
    try {
        ...
        if (!walletAddress || !tokenCA || sellAmount === undefined || currentPrice === undefined) {
            return NextResponse.json(
                { error: 'Missing required fields (wallet, tokenCA, sellAmount, currentPrice)' },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { success: true, message: 'Sell order processed', holding: portfolioItem, transaction },
            { status: 200 }
        );
    } catch (error: any) {
        console.error(`Error processing sell for user ${params.wallet}:`, error);
        return NextResponse.json(
